...
test('advertises Native OAuth helpers and searches all 97 operations over stdio', async () => {
  ...
  try {
    ...
    assert.deepEqual(listed.tools.map(tool => tool.name).sort(), ['complete_oauth', 'get_tool_schema', 'invoke_tool', 'search_tools', 'start_oauth', 'upload_local_file', 'verify_connection']);
...
test('documents startup, OAuth environment, and probe host', async () => {
  ...
  assert.match(example, /ADOBE_REDIRECT_URI=/);
